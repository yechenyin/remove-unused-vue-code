...
import { Expression, SpreadElement, ArrayExpression } from '@babel/types'
import * as compiler from 'vue-template-compiler'
const { getConfiguration, clone } = require('../utils/index')

function parseUnusedStyle(code: string) {
  ...
  const stack: Array<SelectorNode> = []
  const scopes: Array<SelectorNode> = []
  stack.push({
    children: []
  })
  let start = 0
    ...
    if (char == ';') {
      ...
      while (match = seperator.exec(selector)) {
        ...
        if (match[0] == '.' || match[0] == '[') {
          ...
          selectors.push(selector.substring(begin, match.index))
          console.log(match)
          if (/^\s+$/.test(match[0])) { selectors.push(' ') }
          else { selectors.push(match[1]) }
  ...
  scopes.forEach((scope, index) => {
    let selectors_arr: Array<Array<string>> = [[]]
  ...
  function findMatchedNode(node: compiler.ASTNode, selectors: Array<string>) {
    function isNodeMatched(node: compiler.ASTNode, selector: string) {
      node = <compiler.ASTElement>node
      // console.log(selector, node.attrsMap)
      if (selector[0] == '#') {
        if (node?.attrsMap?.id == selector.substr(1)) { return true }
      } else if (selector[0] == '.') {
        let classes = node?.attrsMap?.class?.split(/\s+/)
        if (classes?.includes(selector.substr(1))) { return true }
        //支持动态class变量
        ...
        if (code) {
          ...
          traverse(ast, {
            ...
            ArrayExpression(path) {
              let elements: any[] = path.node.elements
            ...
            ConditionalExpression(path) {
              ...
              if ((node.alternate as any).value == selector.substr(1) || (node.consequent as any).value == selector.substr(1)) {
                found = true
              }
            },
          })
        ...
        if (match = selector.match(/\[\s*([\w-]+)\s*=\s*['"]([^'"]*)['"]\s*]/)) {
          ...
          if (attr && node?.attrsMap?.[attr] === val) { return true }
        } else if (match = selector.match(/\[\s*(\S+)\s*]/)) {
          let [, attr] = match
          if (attr && node.attrsMap && attr in node.attrsMap) { return true }
        }
      } else {
    ...
    let found = false
    function searchMatchedNode(node: compiler.ASTNode, selectors: Array<string>, matched = 0) {
      node = <compiler.ASTElement>node
      //忽略伪元素选择器
      ...
      if (found || !selector || !node || !selectors.length) { return }
      ...
      if (selectors[matched + 1]?.[0] == '~' || selectors[matched + 1]?.[0] == '+') {
        ...
        if (children) {
          for (let node of children) {
            if (!foundMatched) {
              foundMatched = isNodeMatched(node, selector)
              console.log((<compiler.ASTElement>node).attrsMap, selector, foundMatched)
            } else {
              console.log('sibling is found', node)
              //如果下一个选择器是+,且非空白文本节点，只继续搜索下一个兄弟节点
              if (selectors[matched + 1][0] == '+' && !(node.type === 3 && /^\s+$/.test(node.text))) { break }
            }
          }
          if (!foundMatched) {
            children?.forEach(node => {
              searchMatchedNode(node as compiler.ASTElement, selectors, matched)
            })
          }
        }
      } else if (selector[0] == '*') {
        ...
        if (isNodeMatched(node, selector)) {
          ...
          if (selectors[matched + 1]?.[0] == '>') {
            node.children?.forEach(node => {
              if (isNodeMatched(node, selectors[matched + 2])) {
  ...
  let whole_scopes: Array<Scope> = []
  scopes.forEach((scope, i) => {
    scope.referenced = findMatchedNode(template_ast as compiler.ASTElement, scope.selectors as Array<string>)
    if (!scope.referenced) {
      removes.push(scope as Scope)
      if (scope.wholeEnd) { whole_scopes.push({ start: scope.wholeStart!, end: scope.wholeEnd }) }
    }
  })
  console.log(scopes)
  whole_scopes.forEach((item, i) => {
    ...
    scopes.forEach((scope, i) => {
      if (scope.wholeEnd == item.end && scope.referenced) { whole_no_use = false }
    })
    if (whole_no_use) { removes.push(item) }
  })
